...
describe('computePublicHomepagePayload', () => {
  ...
  it('patches only the monitors touched by the current scheduler batch', () => {
    const dayStart = 1_728_000_000;
    ...
    const baseSnapshot = {
      ...
      monitors: [
        ...
        },
        {
          ...
          uptime_30d: { uptime_pct: 99.99 },
          uptime_day_strip: {
            day_start_at: [dayStart],
            downtime_sec: [0],
            unknown_sec: [0],
            uptime_pct_milli: [99_990],
          },
        },
      ],
      active_incidents: [],
    ...
    expect(patched?.overall_status).toBe('down');
    expect(patched?.monitors[0]?.status).toBe('down');
    expect(patched?.monitors[0]?.last_checked_at).toBe(now);
    expect(patched?.monitors[1]).toEqual(baseSnapshot.monitors[1]);
  });

  it('counts stale-down segments as unknown during fast patching', () => {
    const dayStart = 1_728_000_000;
    const baseNow = dayStart + 120;
    const now = dayStart + 180;
    const baseSnapshot = {
    ...
    expect(patched).not.toBeNull();
    expect(patched?.monitors[0]?.uptime_day_strip.downtime_sec).toEqual([0]);
    expect(patched?.monitors[0]?.uptime_day_strip.unknown_sec).toEqual([60]);
  });
